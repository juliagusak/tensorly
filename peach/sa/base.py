...
################################################################################
from numpy import exp, abs, array, isnan, where, logical_or
...
# Helper functions
################################################################################
def sample_uniform(low=0, high=1, fmt='f'):
   '''
   Sample uniform floats or integers depending on the format ``fmt``.
   '''
   if fmt[-1] in 'fd':
       return uniform(low, high)
   elif fmt[-1] in 'iIcbBhHlL':
...
class BinarySA(object):
    ...
    def __init__(self, f, x0, ranges=[ ], fmt=None, neighbor=InvertBitsNeighbor,
                 T0=1000., rt=0.95, emax=1e-8, imax=1000, neighbor_params = None):
        '''
        Initializes the optimizer.
        ...
        try:
            ...
            if neighbor_params is not None:
                neighbor = neighbor(**neighbor_params)
            else:
                neighbor = neighbor()
        except TypeError:
            pass
    ...
    def __encode(self, values):
        ...
        if self.format is None:
            ...
            x = bitarray()
            x.frombytes(struct.pack(self.format, *values))
            return x[:]
    ...
    def __decode(self, bits):
        ...
        if self.format is None:
            return bits[:]
        return struct.unpack(self.format, bits.tobytes())
    ...
    def step(self):
        ...
        *or* if the temperature is high enough. In case it is not accepted, the
        ...
        if r is not None:
            x0 = r[:, 0]
            x1 = r[:, 1]
            need_replacement = logical_or(xs < x0, xs > x1)
            need_replacement = logical_or(need_replacement, isnan(xs))
            if any(need_replacement):
                xs = [ sample_uniform(r0, r1, self.format)
                       if replace else xi
                       for (r0, r1), xi, replace in zip(
                               r, xs, need_replacement) ]
